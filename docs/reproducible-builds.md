## Build Spend Deterministically

Gitian is the deterministic build process that is used to build the Spend executables. It provides a way to be reasonably sure that the executables are really built from the git source. It also makes sure that the same, tested dependencies are used and statically built into the executable.
...
This page contains all instructions required to build and sign reproducible Spend binaries for Linux, Mac OS X, and Windows.
...
```
git clone git@github.com:cosmos/spend
```
...
* `spend.sigs` containing the signed build report.
...
```
cd spend/ && ./contrib/gitian-build.sh -s user@example.com all
```

If you want to generate unsigned builds, just remove the option `-s` from the command line:

```
./cmd/spend/contrib/gitian-build.sh linux
```

At the end of the procedure, build results can be found in the `./spend.sigs` directory:

Please refer to the `contrib/gitian-build.sh`'s help screen for further information on its usage.

## Signatures upload

Once signatures are generated, they could be uploaded to spend's dedicated repository: https://github.com/spend/Spendchain.sigs.
